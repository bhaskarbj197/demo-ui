...
export class BulkCalendarUploadComponent implements OnInit {
  ...
  bulkProcessedData: Array<BulkCalendarMasterModel> = [];
  bulkValidatedData: Array<BulkCalendarMasterModel> = [];
  bulkDataInput: Array<BulkCalendarMasterModel> = [];
  bulkValidatedDataAll: Array<BulkCalendarMasterModel> = [];
  ...
  private processBulkCalendarMaster() {
    ...
    this.businessLoaderService.bulkUploadBusinessService.processBulkCalendarMasterTemplateFile(request).subscribe(res => {
      this.bulkProcessedData = this.businessLoaderService.bulkUploadBusinessService.getBulkCalendarMasterData(res.body.data.dataResult);
  ...
  onNextClick(){
    ...
    if(this.activeStep === 3) {
      this.processBulkCalendarMaster();
    }
  }

  onTemplateDownloadClick(){
    this.businessLoaderService.bulkUploadBusinessService.downloadBulkTemplateFile(this.enumLoaderService.bulkTemplateTypes.CALENDAR_MASTER).subscribe(res => {
      this.generalUtility.download(res.body, this.enumLoaderService.bulkTemplateFileNames.CALENDAR_MASTER + "." +
  ...
  onValidateDataClick(){
    ...
    if(!this.generalUtility.isEmptyOrUndefined(this.uploadedFile)) {
      request.fileName = this.uploadedFile;
      this.businessLoaderService.bulkUploadBusinessService.validateBulkCalendarMasterTemplateFile(request).subscribe(res => {
        this.bulkDataInput = res.body.data.dataInput;
        this.bulkValidatedDataAll = this.businessLoaderService.bulkUploadBusinessService.getBulkCalendarMasterData(res.body.data.dataInput);
